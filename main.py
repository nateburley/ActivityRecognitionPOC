...
if __name__ == '__main__':
    ...
    X_data, y_data =  data_mod.load_preprocessed_dataset('wisdm_preprocessed_watch_accel.csv')

    # Build training and testing sets
    X_train, X_test, y_train, y_test = data_mod.train_test_split(X_data,y_data,train_size=0.75, test_size=0.25,shuffle=True,stratify=y_data)
    ...
    if TRAIN_NEW:
        ...
        # Train the Extra Trees Classifier, (uses grid search to find optimal parameters)
        model = train_mod.train_extra_trees(model, X_train, y_train)

    else:
        print("Loading model...")

        # Open pickle file with existing model
        model_file = open('best_extra_trees.pickle', 'rb')

        # Load model
        model = pickle.load(model_file)

    # Lastly, evaluate the model
    train_mod.evaluate_model(model, X_test, y_test)